...
<div class="mt-5 md:mt-0 md:col-span-2">
    <form action="{{ route('admin.subcategories.update', $sub_category->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col w-full p-10 px-8 pt-6 mx-auto my-6 mb-4 transition duration-500 ease-in-out transform bg-gray-300 border rounded-lg lg:w-1/2 ">
        @csrf
        @method('PUT')
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          ...
          <div class="grid grid-cols-3 gap-6">
            <div class="col-span-3 sm:col-span-2">
              ...
              <div class="mt-1 flex rounded-md shadow-sm">
                <input type="text" name="name" id="name" value="{{ $sub_category->name }}" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300" placeholder="Category Title">
              </div>
          ...
          <div>
            ...
            <div class="w-full m-2 p-2">
              <img class="h-50 w-50" src="{{ Storage::url($sub_category->image) }}" />
            </div>
